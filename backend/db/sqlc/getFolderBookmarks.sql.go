// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.22.0
// source: getFolderBookmarks.sql

package db

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

const getFolderBookmarks = `-- name: GetFolderBookmarks :many
select id, title, bookmark, host, favicon, thumbnail, notes, user_id, added, updated, deleted, folder_id, beautified, fromchrome from bookmark where user_id = $1 and folder_id = $2 and deleted is null and beautified is null order by added desc
`

type GetFolderBookmarksParams struct {
	UserID   string      `json:"user_id"`
	FolderID pgtype.Text `json:"folder_id"`
}

func (q *Queries) GetFolderBookmarks(ctx context.Context, arg GetFolderBookmarksParams) ([]Bookmark, error) {
	rows, err := q.db.Query(ctx, getFolderBookmarks, arg.UserID, arg.FolderID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []Bookmark{}
	for rows.Next() {
		var i Bookmark
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Bookmark,
			&i.Host,
			&i.Favicon,
			&i.Thumbnail,
			&i.Notes,
			&i.UserID,
			&i.Added,
			&i.Updated,
			&i.Deleted,
			&i.FolderID,
			&i.Beautified,
			&i.Fromchrome,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}
