...
func GetUserRootBookmarks(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	b, err := q.GetUserRootBookmarks(ctx, payload.UserID)
	if err != nil {
		log.Printf("could not get user root bookmarks: %v", err)
		utils.Response(w, "somthing went wrong", 500)
		return
	}
