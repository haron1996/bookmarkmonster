...
func UpdateFolder(w http.ResponseWriter, r *http.Request) {
	...
	f, err := q.UpdateFolder(ctx, sqlc.UpdateFolderParams{FolderName: req.Name, FolderDescription: req.Description, FolderID: req.ID, UserID: payload.UserID})
	if err != nil {
		log.Printf("could not update folder: %v", err)
		utils.Response(w, "something went wrong", 500)
		return
	}
