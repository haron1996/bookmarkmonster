...
func ReadSupportMessages(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	sm, err := sqlc.New(pool).ReadSupportMessages(r.Context())
	if err != nil {
		log.Printf("could not read support messages: %v", err)
		utils.Response(w, "something went wrong", 500)
		return
	}

	utils.JsonResponse(w, sm)
}
