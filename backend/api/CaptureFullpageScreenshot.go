...
type captureFullpageScreenshotReq struct {
	BookmarkID string `json:"bookmark_id"`
}

func CaptureFullpageScreeshot(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	var req *captureFullpageScreenshotReq
	...
	byte := page.MustScreenshotFullPage()

	f, err := os.Create("fullPageScreenShot.png")
	if err != nil {
	...
	url, err := vultr.UploadImage(f, "fullpage-screenshots")
	if err != nil {
		log.Printf("could not upload image: %v", err)
		utils.Response(w, "something went wrong", 500)
		return
	}
